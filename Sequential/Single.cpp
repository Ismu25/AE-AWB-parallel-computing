...
double calculateTotalExposition(ImageBuffer* imageBuffer, ImageBlockSet* imageBlockSet) {
...
ImageBuffer* processExposureCorrection(ImageBuffer* imageBuffer, ImageBlockSet * imageBlockSet, ImageBuffer* imageFusionTest) {
    ...
    double exposure = calculateTotalExposition(imageBuffer, imageBlockSet);
    if (verbose)
        std::cout << "Result of exposure calculation: " << exposure <<std::endl;
    
    ImageBuffer* imageRes = NULL;
    if (imageFusionTest != NULL) { // Si se quiere realizar fusión
        if (verbose)
            std::cout << "Starting image fusion" << std::endl;
        ...
        if (verbose)
            std::cout << "No second image passed, ignoring step" << std::endl;
...
ImageBuffer* processColorCorrection(ImageBuffer* imageBuffer, ImageBlockSet* imageBlockSet) {
    ...
    if (verbose)
        std::cout << "Calculating weighted average and corrections" << std::endl;
    ...
    if (verbose)
        std::cout << "Deviation sum for channel Red: " << deviationSumR << std::endl << "Weighted Avarege for channel Red: " << stdAvgR << std::endl << "Correction Value for channel Red: " << corrR << std::endl
                  << "Deviation sum for channel Green: " << deviationSumG << std::endl << "Weighted Avarege for channel Green: " << stdAvgG << std::endl << "Correction Value for channel Green: " << corrG << std::endl
                  << "Deviation sum for channel Blue: " << deviationSumB << std::endl << "Weighted Avarege for channel Blue: " << stdAvgB << std::endl << "Correction Value for channel BLue: " << corrB << std::endl;

    if (verbose)
        std::cout << "Applying color corrections" << std::endl;
    ImageBuffer* imageBufferRes = createBuffer(imageBuffer->lineNumber, imageBuffer->lineWidth, 0);
    for (int i = 0; i < imageBlockSet->blockNumber; i++) {
...
ImageBuffer* processImageBuffer(ImageBuffer* imageBuffer, ImageBlockSet* imageBlockSet, ImageBuffer* imageFusionTest, const char* saveStep) {
    if (verbose)
        std::cout << "Starting exposure correction" << std::endl;
    ...
    if (verbose)
        std::cout << "Starting color correction" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (fileInput2 != NULL) {
        ...
        if (image2 == NULL) {
            ...
            if (verbose) {
                ...
                std::cout << "Channels : " << channels2 << std::endl;
            }
        }
    ...
    std::cout << "Tiempo Secuencial: " << duration.count() << " segundos" << std::endl;
    // Guardado de resultados
    int result = stbi_write_png(fileOutput, width, height, channels, imRes->mappedData, width * channels);
    if (result) {
        if (verbose)
            std::cout << "Completed image transformation" << std::endl;
    }
    else {
